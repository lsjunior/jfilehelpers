...
public class FieldAlignBean {
  ...
  public static FieldAlignBean createFromAnnotation(final FieldAlign ann, final Field fi) {
    FieldAlignBean fab = new FieldAlignBean();
    if (ann != null) {
      fab.mode = ann.alignMode();
      fab.alignChar = ann.alignChar();
    } else {
      if ((fi.getType() == Integer.class) || (fi.getType() == Float.class)
          || (fi.getType() == Double.class)) {
