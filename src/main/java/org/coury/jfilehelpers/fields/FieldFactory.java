...
public class FieldFactory {
  ...
  public static FieldBase createField(final Field fi, final Class recordClass,
      final boolean someOptional) {
     ...
     * 
     * if (attributes.Length > 1) throw new BadUsageException("The field: " + fi.Name +
     * " has more than one FieldAttribute (left only one or none)");
     * 
     * if (attributes.Length == 0 && recordAttribute is FixedLengthRecordAttribute) throw new
     * BadUsageException(
     * "The record class marked with the FixedLengthRecord attribute must include a FixedLength attribute in each field."
     * );
     * 
     * if (recordAttribute is DelimitedRecordAttribute && fi.IsDefined(typeof (FieldAlignAttribute),
     * true)) throw new BadUsageException(
     * "The AlignAttribute is only valid for fixed length records and are used only for write purpouse."
     * );
     */
    ...
    if (fi.isAnnotationPresent(FieldFixedLength.class)) {
      ...
      FixedLengthRecord fixedLengthRecord =
          (FixedLengthRecord) recordClass.getAnnotation(FixedLengthRecord.class);
      FieldFixedLength attb = fi.getAnnotation(FieldFixedLength.class);
      FieldAlign align = fi.getAnnotation(FieldAlign.class);

      res = new FixedLengthField(fi, attb.value(), FieldAlignBean.createFromAnnotation(align, fi),
      ...
      // default
      DelimitedRecord delimitedAnn =
          ((DelimitedRecord) recordClass.getAnnotation(DelimitedRecord.class));
      String delimiter = ",";
